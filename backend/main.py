...
def create_vertical_video(tracks, scores, pyframes_path, pyavi_path, audio_path, output_path, framerate=25):
    ...
    for tidx, track in enumerate(tracks):
        ...
        for fidx, frame in enumerate(track["track"]["frame"].tolist()):
            ...
            faces[frame].append({'track': tidx, 'score': avg_score, 's': track['proc_track']
                                ["s"][fidx], 'x': track['proc_track']["x"][fidx], 'y': track['proc_track']["y"][fidx]})

    temp_video_path = os.path.join(pyavi_path, "video_only.mp4")

    vout = None
    for fidx, fname in tqdm(enumerate(flist), total=len(flist), desc="Creating verticle video"):
...
def create_subtitles_with_ffmpeg(transcript_segments: list, clip_starts: float, clip_ends: float, clip_video_path: str, output_path: str, max_word: int = 5):
    temp_dir = os.path.dirname(output_path)
    subtitle_path = os.path.join(temp_dir, "temp_subtitles.ass")

    clip_segments = [segment for segment in transcript_segments if segment.get(
        "start") is not None and segment.get("end") is not None and segment.get("end") > clip_starts and segment.get("start") < clip_ends]
    ...
    for segment in clip_segments:
        ...
        start_rel = max(0.0, seg_start - clip_starts)
        end_rel = max(0.0, seg_end - clip_starts)
...
def process_clip(base_dir: str, original_video_path: str, s3_key: str, start_time: float, end_time: float, clip_index: int, transcript_segments: list):
    ...
    create_vertical_video(tracks, scores, pyframes_path,
                          pyavi_path, audio_path, vertical_mp4_path)
    cvv_end_time = time.time()
    print(
        f"Clip {clip_index} vertical video creation time: {cvv_end_time - cvv_start_time:.2f} seconds")

    create_subtitles_with_ffmpeg(transcript_segments, start_time,
                                 end_time, vertical_mp4_path, subtitle_output_path, max_word=5)

    s3_client = boto3.client("s3")
    s3_client.upload_file(subtitle_output_path,
                          "clipper-bucket01", output_s3_key)
...
class Clipper:
    ...
    def process_video(self, request: ProcessVideoRequest, token: HTTPAuthorizationCredentials = Depends(auth_scheme)):
        ...
        # 2. Identify viral moments for clips
